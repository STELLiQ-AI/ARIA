...
extern "C" {
    ...
    // Enable QNN HTP encoder acceleration for this context.
    // Requires QNN to be initialized externally via aria::qnn::initialize() and
    // context binary loaded via aria::qnn::loadContextBinary().
    // Returns 0 on success, 1 if QNN is not enabled in build, -1 on error.
    WHISPER_API int whisper_ctx_enable_qnn_encoder(struct whisper_context * ctx);
    WHISPER_API int whisper_ctx_enable_qnn_encoder_with_state(
        struct whisper_context * ctx,
        struct whisper_state * state);
